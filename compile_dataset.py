...
class_mapping = {'man': 0, 'boy': 1, 'house': 2, 'woman': 3, 'girl': 4, 'table': 5, 'road': 6, 'horse': 7,
                 'dog': 8, 'ship': 9, 'bird': 10, 'mountain': 11, 'bed': 12, 'train': 13, 'bridge': 14, 'fish': 15,
                 'cloud': 16, 'chair': 17, 'cat': 18, 'baby': 19, 'castle': 20, 'forest': 21, 'television': 22,
                 'bear': 23, 'camel': 24, 'sea': 25, 'fox': 26, 'plain': 27, 'bus': 28, 'snake': 29, 'lamp': 30,
                 'clock': 31, 'lion': 32, 'tank': 33, 'palm': 34, 'rabbit': 35, 'pine': 36, 'cattle': 37, 'oak': 38,
                 'mouse': 39, 'frog': 40, 'ray': 41, 'bicycle': 42, 'truck': 43, 'elephant': 44, 'roses': 45,
                 'wolf': 46, 'telephone': 47, 'bee': 48, 'whale': 49}
...
def generate_audio_modality(audio_csv_path, audio_test_csv, audio_data_path, features_path, compactness=0,
    ...
    train_data = data[data['label'].isin(ID_classes)]
    ...
    if add_noise_test:
        test_data = add_noise_to_audio(test_data, audio_data_path, 'noisy_audio', min_snr=min_snr, max_snr=max_snr,
                                       noisy_data_ratio=noisy_data_ratio)
    ood_data = data[~data['label'].isin(ID_classes)]
    if add_noise_test:
...
def generate_text_modality(text_csv_path, text_test_csv, features_path, compactness=0, num_sampling=10,
    ...
    if os.path.exists(text_test_csv):
        ...
        train_data, test_data = generate_test_split(train_data, test_count_per_label=100, features_path=features_path)
        test_data.to_csv(text_test_csv)
    if add_noise_train:
